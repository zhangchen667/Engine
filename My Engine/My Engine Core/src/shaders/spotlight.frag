...
struct Light{
	vec3 position;
	vec3 direction;
	float cutOff;//内切光束角的余弦值
	float outerCutOff;//外切光束角的余弦值
...
void main(){
	...
	//聚光衰减
	float theta=dot(lightDir,normalize(-light.direction));//计算片段位置与聚光方向的夹角余弦值
	float difference=light.cutOff-light.outerCutOff;//内外切光束角余弦值之差
	float intensity=clamp((theta-light.outerCutOff)/difference,0.0,1.0);//计算强度
	diffuse*=intensity;
	specular*=intensity;
	//距离衰减
