	...
	namespace opengl {
		Framebuffer::Framebuffer(int width, int height, bool multisampledBuffers) : m_Width(width), m_Height(height)
		{
			...
			if (multisampledBuffers) {//如果使用多重采样缓冲
				glRenderbufferStorageMultisample(GL_RENDERBUFFER, MSAA_SAMPLE_AMOUNT, GL_DEPTH24_STENCIL8, width, height);//分配渲染缓冲存储空间，使用多重采样
			}
			else {
				glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH24_STENCIL8, width, height);//分配渲染缓冲存储空间
			}
				// 创建颜色缓冲纹理
